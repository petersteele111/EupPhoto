<x-app-layout>
    <div class="px-2 pt-16 w-100">
        <div class="popular-photos" id="next-section">
            ...
            <div class="flex flex-col lg:flex-row flex-wrap justify-center space-y-4 lg:space-y-0 mb-8">
                <button class="text-white text-sm lg:text-3xl bg-gray-800 px-4 py-2 rounded hover:bg-gray-700 lg:mb-0 mx-2" data-filter="*">Show All</button>
                @foreach($categories as $category)
                    @if(!empty($category->category))
                        <button class="text-white text-sm lg:text-3xl bg-gray-800 px-4 py-2 rounded hover:bg-gray-700 mb-2 lg:mb-0 mx-2" data-filter=".{{ $category->category }}">{{ ucfirst($category->category) }}</button>
                    @endif
                @endforeach
            </div>
            <div class="grid">
